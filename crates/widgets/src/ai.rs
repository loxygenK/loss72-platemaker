...
impl AiUsage {
    pub fn description(&self) -> (&str, &str) {
        match self {
            ...
            AiUsage::NonText => ("AI generated non-text contents", "テキスト以外のコンテンツで AI を使っています"),
            AiUsage::ResearchSupport => ("AI supported researching for this", "この記事を書くにあたって、AI と協力して調査しました"),
            AiUsage::Research => ("AI researched for this", "この記事を書くにあたって、AI に調査してもらいました"),
            AiUsage::ArticleOutlining => ("AI generated the outline", "記事の構成作成に AI を使っています"),
...
impl Widget for AiUsage {
    ...
    fn build(&self) -> String {
        ...
        let heavy_class = if self.heavy_use() { "aiusage-heavy" } else { "" };
